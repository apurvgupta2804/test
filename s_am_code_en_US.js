...
/************************ ADDITIONAL FEATURES ************************
     ...
     Plugins
*/
var s_am=s_gi(s_am_account)

/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */

/* Conversion Config */
s_am.currencyCode="USD"
...
function s_am_doPlugins(s_am) {
	...
	/* set eVar5 from prop4 and de-duplicate*/
	if(!s_am.eVar5){
		s_am.eVar5=s_am.getValOnce(s_am.prop4,"s_v5",0);
	}
	/* set browse finding method */
	if(s_am.eVar5)
		s_am.eVar14="Browse";
		
	/* Set external campaigns based on query string parameter extid and de-duplicate*/
	if(!s_am.campaign){
		s_am.campaign=s_am.getQueryParam('extid');
		s_am.campaign=s_am.getValOnce(s_am.campaign,'s_v0',0);
	}
	/* if extid parameter is not present, check for traffic_src and populate to campaigns */
	if(!s_am.campaign){
		s_am.campaign=s_am.getQueryParam('traffic_src');
		s_am.campaign=s_am.getValOnce(s_am.campaign,'s_v0',0);
	}

	/* check for utm_source */
	if(!s_am.eVar27)
		s_am.eVar27=s_am.getQueryParam('utm_source')

	/* check for traffic_src */
	if(!s_am.eVar27)
		s_am.eVar27=s_am.getQueryParam('traffic_src')

	/* collect utm medium parameter if available */
	if(!s_am.eVar28)
		s_am.eVar28=s_am.getQueryParam('utm_medium');

	/* set external campaigns finding method and clear merchandising category */
	if(s_am.campaign && typeof s_am.campaign != 'undefined'){
		s_am.eVar14="External Campaigns";
		s_am.eVar5="Non-Browse";
	}
	/* Set internal campaigns based on query string parameter intid */
	if(!s_am.eVar1){
		s_am.eVar1=s_am.getQueryParam('intid');
	}
	/* set internal campaigns finding method and clear merchandising category */
	if(s_am.eVar1 && typeof s_am.eVar1 != 'undefined'){
		s_am.eVar14="Internal Campaigns";
		s_am.eVar5="Non-Browse";
	}
	/* Force search term to lower case and copy to eVar2 */
	if(s_am.prop2) {
		s_am.prop2=s_am.eVar2=s_am.prop2.toLowerCase();
		s_am.prop2=s_am.getValOnce(s_am.prop2,'s_p2',0); //de-duplicate search term prop
		s_am.eVar14="Search"; //set product finding method
		s_am.eVar5="Non-Browse"; //clear merchandising category
	}
	/*
	...
	*/
	...
	if(t_search && !s_am.products)
		s_am.products=";";

	/* Set custom prodView event */
	var s_prodViewCheck=s_am.events.indexOf("prodView");
	if(s_prodViewCheck >= 0)
		s_am.events=s_am.events+',event4';
	
	/* set eVar3 based on purchaseID */
	if (s_am.purchaseID && !s_am.eVar3) 
		s_am.eVar3=s_am.purchaseID;
	
	/* set eVar6 based on s_am.zip */
	if(s_am.zip && !s_am.eVar6)
		s_am.eVar6=s_am.zip;
	
	/* set eVar7 based on s_am.state */
	if(s_am.state && !s_am.eVar7)
		s_am.eVar7=s_am.state;
	
	/* strip dollar sign from product string */
	if(s_am.products)
		s_am.products = s_am.repl(s_am.products,"$","");

	/* set prop1 to channel if not already populated */
	if (!s_am.prop1) 
		s_am.prop1=s_am.channel;
	...
	s_am.events=s_am.getCartOpen("s_scOpen");
	
	/* set eVar11 based on prop11 */
	s_am.eVar11=s_am.getValOnce(s_am.prop11,"s_v11",0)
	
	/* set eVar19 based on prop12 */
	s_am.eVar19=s_am.getValOnce(s_am.prop12,"s_v18",0)
	
	/* set automatic cart open */
	s_am.events=s_am.getCartOpen("s_scOpen");
	
	/* get timeparting PST */
	s_am.eVar24=s_am.getTimeParting('h','-8',new Date().getFullYear()); // Set hour 
	s_am.eVar25=s_am.getTimeParting('d','-8',new Date().getFullYear()); // Set day
	s_am.eVar26=s_am.getTimeParting('w','-8',new Date().getFullYear()); // Set Weekend / Weekday
}
...
s_am.clearVars=new Function("l","f",""
...
+"lse}");
s_am.p_clr=new Function("t","var s=this;s[t]=''");
 ...
 */
s_am.getCartOpen=new Function("c",""
 ...
 */
s_am.getAndPersistValue=new Function("v","c","e",""
...
s_am.getVisitFlag=new Function("c",""
 ...
 */
s_am.getTimeParting=new Function("t","z","y",""
 ...
 */
if(!s_am.__ccucr){s_am.c_rr=s_am.c_r;s_am.__ccucr = true;
s_am.c_r=new Function("k",""
 ...
 */
if(!s_am.__ccucw){s_am.c_wr=s_am.c_w;s_am.__ccucw = true;
s_am.c_w=new Function("k","v","e",""
+"this.new2 = true;"
...
s_am.visitorNamespace="amazonmerchants";
s_am.dc="122"
